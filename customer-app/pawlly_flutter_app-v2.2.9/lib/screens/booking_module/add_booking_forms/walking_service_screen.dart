                  ...
                  onTap: () async {
                    if (walkingServiceController.dateCont.text.trim().isEmpty) {
                      ...
                      TimeOfDay? pickedTime = await showTimePicker(
                        initialTime: TimeOfDay.fromDateTime(walkingServiceController.bookWalkingReq.time.dateInHHmm24HourFormat),
                        context: context,
                      );
                      if (pickedTime != null) {
                              ...
                              decoration: boxDecorationDefault(
                                color: walkingServiceController.selectedDuration.value == durationList[index]
                                    ? isDarkMode.value
                                        ...
                                        : lightPrimaryColor
                                    : context.cardColor,
                                border: walkingServiceController.selectedDuration.value == durationList[index] ? Border.all(color: primaryColor.withValues(alpha: 0.15)) : null,
                              ),
                              child: Text(
      ...
      itemBuilder: (context, index) {
        return SettingItemWidget(
          ...
          onTap: () {
            ...
            walkingServiceController.getWalker();
            walkingServiceController.getDurationList(employeeId: walkingServiceController.selectedWalker.value.id, isBooking: 1);
            walkingServiceController.showBookBtn(false);
            Get.back();
          },
        );
