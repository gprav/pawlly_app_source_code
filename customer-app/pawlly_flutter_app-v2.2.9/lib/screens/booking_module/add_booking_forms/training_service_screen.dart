...
class TrainingServiceScreen extends StatelessWidget {
  ...
  TrainingServiceScreen({super.key, this.isFromReschedule = false});
                              ...
                              serviceCommonBottomSheet(
                                ...
                                child: Obx(
                                  () => BottomSelectionSheet(
                                    ...
                                    searchApiCall: (p0) {
                                      trainingController.getTrainer(searchtext: p0);
                                    },
                                    onRetry: () {
                            ...
                            serviceCommonBottomSheet(context,
                                child: Obx(
                                  () => BottomSelectionSheet(
                                    ...
                                    isEmpty: !trainingController.isLoading.value && trainingController.trainingList.isEmpty,
                                    ...
                                    listWidget: Obx(() => trainingListWid(trainingController.trainingList).expand()),
                                  ),
                                ));
                          },
                          decoration: inputDecoration(
        ...
        return Column(
          children: [
            SettingItemWidget(
              ...
              subTitle: (trainingData.createdBy == trainingController.selectedTrainer.value.id) ? '${locale.value.providedBy} ${trainingController.selectedTrainer.value.fullName}' : locale.value.providedByAdmin,
              subTitleTextStyle: secondaryTextStyle(color: primaryColor, fontStyle: FontStyle.italic),
              onTap: () {
                ...
                trainingController.getTrainingDurationList(trainingTypeId: trainingController.selectedTraining.value.id);
                trainingController.showBookBtn(false);
                Get.back();
              },
            ),
            if (index < list.length - 1) commonDivider.paddingSymmetric(vertical: 6),
          ],
        );
      },
      onNextPage: () {
      ...
      itemBuilder: (context, index) {
        return SettingItemWidget(
          ...
          onTap: () {
            ...
            trainingController.getTraining(employeeId: trainingController.selectedTrainer.value.id, isBooking: 1);
                  ...
                  onTap: () async {
                    if (trainingController.dateCont.text.trim().isEmpty) {
                      ...
                      TimeOfDay? pickedTime = await showTimePicker(
                        initialTime: TimeOfDay.fromDateTime(trainingController.bookTrainingReq.time.dateInHHmm24HourFormat),
                        context: context,
                      );
                        ...
                        return Obx(
                          () => InkWell(
                            onTap: () {
                              ...
                              currentSelectedService.value.serviceAmount = trainingController.selectedDuration.value.amount.toDouble();
                              trainingController.bookTrainingReq.price = trainingController.selectedDuration.value.amount.toDouble();
                              trainingController.showBookBtn(false);
