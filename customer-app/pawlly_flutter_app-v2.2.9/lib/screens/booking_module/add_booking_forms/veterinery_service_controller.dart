...
class VeterineryController extends GetxController {
  ...
  //Get Category List
  void getCategory({String searchtext = ""}) {
    isLoading(true);
  ...
  ///Get Service List
  void getService({String searchtext = ""}) {
    isLoading(true);
    PetServiceFormApis.getService(
      ...
      if (isUpdateForm.value) {
        selectedService(serviceList.firstWhere((p0) => p0.id == bookingFormData.value.veterinaryServiceId, orElse: () => ServiceModel()));
